...
# found in the DEPS file _for chromium_
# note: pdfium font tests still use an old version for this file
...
EGIT_COMMIT="679c5ca4865cd3de8ee128f523ff793bba3a18d1"
...
SRC_URI="
	...
	test? (
		https://chromium-fonts.storage.googleapis.com/${TEST_FONT}
			-> chromium-testfonts-${TEST_FONT}.tar.gz
	)
"
...
src_unpack() {
	...
	# build/ contains required GN templates/conf and python scripts
	...
	# bundled dragonbox
	EGIT_REPO_URI="https://github.com/jk-jeon/dragonbox.git"
	EGIT_CHECKOUT_DIR="${S}"/third_party/dragonbox/src
	EGIT_COMMIT=$(awk -F\' '$2 == "dragonbox_revision" && NF == 5 {print $4}' "${S}"/DEPS)
	git-r3_src_unpack
...
src_prepare() {
	...
	mkdir -p third_party/fast_float/src/include/ || die
	ln -sf /usr/include/fast_float third_party/fast_float/src/include/ || die

	sed -i 's|script_executable = "//third_party/cpython3/host/bin/python3"|script_executable = "python3"|' .gn || die
}
