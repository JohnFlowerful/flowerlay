...
/*  uniform_rng - Uniform distribution random number generator.
    ...
    distribution due to RAND_MAX typically not being evenly divisible by
    the limit, and worse, the lower bits of typical PRNGs having extremely
...
public:
  ...
  int rand_below(int limit) {
    return this->rand_range(0, limit - 1);
  }
...
int
uniform_rng::rand_range(int lo, int hi) {
  if (lo > hi) {
    throw torrent::input_error(
      "Empty interval passed to rand_range (low > high)");
  }
  if (lo < 0 || RAND_MAX < lo) {
...
*/
torrent::Object
apply_random(rpc::target_type target, const torrent::Object::list_type& args) {
...
// return various scrape information of the "main" tracker for this download
// item
int64_t
get_active_tracker_scrape_info(const int operation, torrent::Download* item) {
...
/* included upstream
  @DOC
    ...
    Configuration example:
        ...
        # VIEW: Show active and incomplete torrents (in view #9) and update
every 20 seconds #       Items are grouped into complete, incomplete, and
queued, in that order. #       Within each group, they're sorted by upload and
then download speed. view.sort_current =
active,"compare=----,d.is_open=,d.complete=,d.up.rate=,d.down.rate=" schedule =
filter_active,12,20,"view.filter =
active,\"or={d.up.rate=,d.down.rate=,not=$d.complete=}\" ;view.sort=active"

torrent::Object apply_compare(rpc::target_type target, const
torrent::Object::list_type& args) { if (!rpc::is_target_pair(target)) throw
torrent::input_error("Can only compare a target pair.");
...
itr++) { std::string field = itr->as_string(); result1 =
rpc::parse_command_single(rpc::get_target_left(target), field); result2 =
rpc::parse_command_single(rpc::get_target_right(target), field);
        ...
        switch (result1.type()) {
            case torrent::Object::TYPE_VALUE:
                if (result1.as_value() != result2.as_value())
                    return (int64_t) (descending ^ (result1.as_value() <
result2.as_value())); break;

            case torrent::Object::TYPE_STRING:
                if (result1.as_string() != result2.as_string())
                    return (int64_t) (descending ^ (result1.as_string() <
result2.as_string())); break;
...
static std::map<int, std::string>
  bound_commands[ui::DownloadList::DISPLAY_MAX_SIZE];
...
/*  @DOC
   ...
   pressed.
        ...
        Configuration example:
            ...
            schedule =
   bind_7,1,0,"ui.bind_key=download_list,7,ui.current_view.set=rtcontrol"
*/
torrent::Object
apply_ui_bind_key(rpc::target_type target, const torrent::Object& rawArgs) {
  ...
  if (keydef.empty() || keydef.size() > (keydef[0] == '0'   ? 4
                                         : keydef[0] == '^' ? 2
                                                            : 1))
    throw torrent::input_error("Bad key definition.");
  int key = keydef[0];
  if (key == '^' && keydef.size() > 1)
    key = keydef[1] & 31;
  if (key == '0' && keydef.size() != 1) {
    if (keydef.size() != 4)
      throw torrent::input_error(
        "Bad key definition (expected 4 digit octal code).");
  ...
  bound_commands[displayType][key] =
    commands; // keep hold of the string, so the c_str() below remains valid
  switch (displayType) {
    case ui::DownloadList::DISPLAY_DOWNLOAD_LIST:
      display->bindings()[key] =
        std::bind(&ui::ElementDownloadList::receive_command,
                       (ui::ElementDownloadList*)display,
  ...
  if (!new_binding && verbose) {
    ...
    msg +=
      " for " + keydef + " in " + element + " with " + commands.substr(0, 30);
    if (commands.size() > 30)
      msg += "...";
...
torrent::Object
cmd_log_messages(const torrent::Object::string_type& arg) {
  if (arg.empty()) {
  ...
  if (!arg.empty()) {
    core::log_messages_fd =
      open(torrent::utils::path_expand(arg).c_str(), O_WRONLY | O_APPEND | O_CREAT, 0644);

    if (core::log_messages_fd < 0) {
      throw torrent::input_error("Could not open message log file.");
    }

    control->core()->push_log_std("Opened message log file '" +
                                  torrent::utils::path_expand(arg) + "'.");
  }
...
torrent::Object
cmd_import_return(rpc::target_type target, const torrent::Object& args) {
  // Handled in src/rpc/parse_commands.cc::parse_command_file via patch
  throw torrent::input_error("import.return");
}

torrent::Object
cmd_do(rpc::target_type target, const torrent::Object& args) {
  return rpc::call_object(args, target);
}

/* included upstream
torrent::Object retrieve_d_custom_if_z(core::Download* download, const
torrent::Object::list_type& args) { torrent::Object::list_const_iterator itr =
args.begin(); if (itr == args.end()) throw
torrent::bencode_error("d.custom.if_z: Missing key argument."); const
std::string& key = (itr++)->as_string(); if (key.empty()) throw
torrent::bencode_error("d.custom.if_z: Empty key argument."); if (itr ==
args.end()) throw torrent::bencode_error("d.custom.if_z: Missing default
argument.");

    try {
        const std::string& val =
download->bencode()->get_key("rtorrent").get_key("custom").get_key_string(key);
...
torrent::Object
cmd_d_custom_set_if_z(core::Download*                   download,
...
torrent::Object
cmd_d_custom_erase(core::Download*                   download,
                   const torrent::Object::list_type& args) {
  for (torrent::Object::list_type::const_iterator itr  = args.begin(),
                                                  last = args.end();
       itr != last;
       itr++) {
    const std::string& key = itr->as_string();
...
/* included upstream
torrent::Object retrieve_d_custom_map(core::Download* download, bool keys_only,
const torrent::Object::list_type& args) { if (args.begin() != args.end()) throw
torrent::bencode_error("d.custom.keys/items takes no arguments.");

    torrent::Object result = keys_only ? torrent::Object::create_list() :
torrent::Object::create_map(); torrent::Object::map_type& entries =
download->bencode()->get_key("rtorrent").get_key("custom").as_map();

    for (torrent::Object::map_type::const_iterator itr = entries.begin(), last =
entries.end(); itr != last; itr++) { if (keys_only)
result.as_list().push_back(itr->first); else result.as_map()[itr->first] =
itr->second;
    }
...
torrent::Object
cmd_d_custom_toggle(core::Download* download, const std::string& key) {
  bool result = true;
  try {
    const std::string& strval = download->bencode()
                                  ->get_key("rtorrent")
                                  .get_key("custom")
                                  .get_key_string(key);
    if (!strval.empty()) {
  ...
  download->bencode()
    ->get_key("rtorrent")
    .insert_preserve_copy("custom", torrent::Object::create_map())
    .first->second.insert_key(key, result ? "1" : "0");
  return (int64_t)(result ? 1 : 0);
}

torrent::Object
retrieve_d_custom_as_value(core::Download* download, const std::string& key) {
  try {
    const std::string& strval = download->bencode()
                                  ->get_key("rtorrent")
                                  .get_key("custom")
    ...
    if (*junk)
      throw torrent::input_error("d.custom.as_value(" + key +
                                 "): junk at end of '" + strval + "'!");
...
arguments."); torrent::Object::list_const_iterator arg = args.begin();
...
torrent::Object::create_list())->as_list();
    ...
    for (torrent::Object::list_const_iterator command = arg; command !=
args.end(); command++) { const std::string& cmdstr = command->as_string();
      row.push_back(rpc::parse_command(rpc::make_target(*item), cmdstr.c_str(),
cmdstr.c_str() + cmdstr.size()).first);
    }
  }
...
cmd_throttle_names() {
  ...
  for (core::ThrottleMap::const_iterator itr =
         control->core()->throttles().begin();
       itr != control->core()->throttles().end();
       itr++) {
...
size_t
u8_length(const std::string& text) {
  // Take total length and subtract number of non-leading multi-bytes
  return text.length() - count_if(text.begin(), text.end(), [](char c) -> bool {
           return (c & 0xC0) == 0x80;
         });
}
...
std::string
u8_chop(const std::string& text, size_t glyphs) {
  std::mbstate_t mbs   = std::mbstate_t();
  size_t         bytes = 0, skip;
  const char*    pos   = text.c_str();

  while (*pos && glyphs-- > 0 &&
         (skip = std::mbrlen(pos, text.length() - bytes, &mbs)) > 0) {
    pos += skip;
    bytes += skip;
  }
...
string_get_first_arg(const char* name, const torrent::Object::list_type& args) {
  torrent::Object::list_const_iterator itr = args.begin();
  if (args.size() < 1 || !itr->is_string()) {
    throw torrent::input_error("string." + std::string(name) +
                               " needs a string argument.0!");
  }
  return itr->as_string();
}
...
static int64_t
string_get_value_arg(const char*                           name,
                     torrent::Object::list_const_iterator& itr) {
  int64_t result = 0;
  if (itr->is_string()) {
    char* junk = 0;
...
torrent::Object
cmd_string_len(rpc::target_type                  target,
               const torrent::Object::list_type& args) {
  std::mbstate_t mbs    = std::mbstate_t();
...
torrent::Object
cmd_string_join(rpc::target_type                  target,
                const torrent::Object::list_type& args) {
  std::string delim = string_get_first_arg("join", args);
  std::string result;
  torrent::Object::list_const_iterator first = args.begin() + 1,
                                       last  = args.end();
...
torrent::Object
cmd_string_strip(int where, const torrent::Object::list_type& args) {
  ...
  if (args.size() == 1) {
    ...
    do {
      changed = false;
      for (torrent::Object::list_const_iterator itr = first; itr != last;
           ++itr) {
        ...
        do {
          ...
          if (where >= 0 && lpos <= rpos - strippable.length()) {
            if (0 == strncmp(text.c_str() + rpos - strippable.length(),
                             strippable.c_str(),
                             strippable.length())) {
...
torrent::Object
cmd_string_pad(bool at_end, const torrent::Object::list_type& args) {
  std::string text;
  ...
  std::string                          filler;
  if (itr != args.end())
    pad_len = string_get_value_arg("[lr]pad(pad_len)", itr);
  if (itr != args.end())
    filler = (itr++)->as_string();
  if (pad_len < 0)
    throw torrent::input_error(
      "string.[lr]pad: Invalid negative padding length!");
  if (filler.empty())
    filler = " ";
...
torrent::Object
cmd_string_split(rpc::target_type                  target,
                 const torrent::Object::list_type& args) {
  ...
  if (delim.length()) {
    ...
    while (*cpos &&
           (skip = std::mbrlen(cpos, text.length() - bytes, &mbs)) > 0) {
...
torrent::Object
cmd_string_substr(rpc::target_type                  target,
                  const torrent::Object::list_type& args) {
  ...
  std::string                          fallback;
  if (itr != args.end())
    glyphs = string_get_value_arg("substr(pos)", itr);
  if (itr != args.end())
    count = string_get_value_arg("substr(count)", itr);
  if (itr != args.end())
    fallback = (itr++)->as_string();
...
torrent::Object
cmd_string_shorten(rpc::target_type                  target,
                   const torrent::Object::list_type& args) {
  ...
  if (itr != args.end())
    maxlen = string_get_value_arg("shorten(maxlen)", itr);
  if (itr != args.end())
    tail = string_get_value_arg("shorten(tail)", itr);

  if (maxlen < 0 || tail < 0) {
    throw torrent::input_error(
      "string.shorten: Invalid negative maximal or tail length!");
  }
  ...
  offsets[idx] = bytes;
  if (tail <= idx)
    tail_bytes = offsets[idx - tail];

  return text.substr(0, head_bytes) + (head + tail < u8len ? "…" : "") +
...
torrent::Object
cmd_string_contains(rpc::target_type                  target,
                    const torrent::Object::list_type& args) {
...
torrent::Object
cmd_string_contains_i(rpc::target_type                  target,
                      const torrent::Object::list_type& args) {
  return apply_string_contains(true, args);
}

torrent::Object
apply_string_mutate(int operation, const torrent::Object::list_type& args) {
  if (args.size() < 1) {
  ...
  for (++itr; itr != args.end(); ++itr) {
    ...
    switch (operation) {
      ...
      case 2:
        for (size_t pos = 0;
             (pos = result.find(needle, pos)) != std::string::npos;
             pos += subst.length()) {
...
torrent::Object
cmd_string_map(rpc::target_type                  target,
               const torrent::Object::list_type& args) {
  return apply_string_mutate(1, args);
}

torrent::Object
cmd_string_replace(rpc::target_type                  target,
                   const torrent::Object::list_type& args) {
  return apply_string_mutate(2, args);
}

torrent::Object
cmd_string_compare(int mode, const torrent::Object::list_type& args) {
  const char* opnames[] = { "equals", "startswith", "endswith" };
  if (args.size() < 2) {
    throw torrent::input_error("string." + std::string(opnames[mode]) +
  ...
  for (torrent::Object::list_const_iterator itr = first; itr != last; ++itr) {
    ...
    switch (mode) {
      case 0:
        ...
        break;
      case 1:
        if (value.substr(0, cmp.length()) == cmp)
          return (int64_t)1;
        break;
      case 2:
        if (value.length() >= cmp.length() &&
            value.substr(value.length() - cmp.length()) == cmp)
          return (int64_t)1;
        break;
      default:
        throw torrent::input_error(
          "string comparison: internal error (unknown mode)");
    }
  }
...
torrent::Object
cmd_array_at(rpc::target_type target, const torrent::Object::list_type& args) {
  if (args.size() != 2) {
...
void
add_capability(const char* name) {
  system_capabilities.insert(name);
}

torrent::Object
cmd_system_has(const torrent::Object::string_type& arg) {
  if (arg.empty()) {
...
cmd_system_has_list() {
  ...
  for (std::set<std::string>::const_iterator itr = system_capabilities.begin();
       itr != system_capabilities.end();
       itr++) {
...
torrent::Object
cmd_system_has_methods(bool filter_public) {
  torrent::Object             result     = torrent::Object::create_list();
  torrent::Object::list_type& resultList = result.as_list();
  ...
  for (rpc::CommandMap::const_iterator itr  = rpc::commands.begin(),
       ...
       itr != last;
       itr++) {
    if (bool(itr->second.m_flags & rpc::CommandMap::flag_public) ==
        filter_public) {
      resultList.push_back(itr->first);
    }
  }
...
torrent::Object
cmd_system_client_version_as_value() {
  int64_t     result = 0;
  const char* pos    = RT_VERSION;

  while (*pos) {
    result = 100 * result + strtol(pos, (char**)&pos, 10);
    if (*pos && *pos != '.')
      throw torrent::input_error(
        "INTERNAL ERROR: Bad version " RT_VERSION);
    if (*pos)
      ++pos;
  }
  return result;
...
/* included upstream as 'apply_value'
torrent::Object cmd_value(rpc::target_type target, const
torrent::Object::list_type& args) { if (args.size() < 1) { throw
torrent::input_error("'value' takes at least a number argument!");
    }
    if (args.size() > 2) {
    ...
    if (args.front().is_value()) {
        ...
        int base = args.size() > 1 ? args.back().is_value() ?
                   args.back().as_value() :
strtol(args.back().as_string().c_str(), NULL, 10) : 10; char* endptr = 0;
...
torrent::Object
cmd_d_tracker_domain(core::Download* download) {
  return get_active_tracker_domain(download->download());
}

torrent::Object
cmd_d_tracker_scrape_info(const int operation, core::Download* download) {
...
inline std::vector<int64_t> as_vector(const torrent::Object::list_type& args) {
    ...
    for (torrent::Object::list_const_iterator itr = args.begin(), last =
args.end(); itr != last; itr++) { if (itr->is_value()) {
...
int64_t apply_math_basic(const char* name, const
std::function<int64_t(int64_t,int64_t)> op, const torrent::Object::list_type&
args) { int64_t val = 0, rhs = 0; bool divides = !strcmp(name, "math.div") ||
!strcmp(name, "math.mod");

    if (args.size() == 0)
        throw torrent::input_error(std::string(name) + ": No arguments
provided!");

    for (torrent::Object::list_const_iterator itr = args.begin(), last =
args.end(); itr != last; itr++) { if (itr->is_value()) { rhs = itr->as_value();
        } else if (itr->is_string()) {
            rhs = rpc::convert_to_value(itr->as_string());
        } else if (itr->is_list()) {
            rhs = apply_math_basic(name, op, itr->as_list());
        } else {
            throw torrent::input_error(std::string(name) + ": Wrong argument
type");
        }

        if (divides && !rhs && itr != args.begin())
            throw torrent::input_error(std::string(name) + ": Division by
zero!"); val = itr == args.begin() ? rhs : op(val, rhs);
    }
...
int64_t apply_arith_basic(const std::function<int64_t(int64_t,int64_t)> op,
                          const torrent::Object::list_type& args) {
    if (args.size() == 0)
        throw torrent::input_error("Wrong argument count in
apply_arith_basic.");

    int64_t val = 0;

    for (torrent::Object::list_const_iterator itr = args.begin(), last =
args.end(); itr != last; itr++) { if (itr->is_value()) { val = itr ==
args.begin() ? itr->as_value() : (op(val, itr->as_value()) ? val :
itr->as_value()); } else if (itr->is_string()) { int64_t cval =
rpc::convert_to_value(itr->as_string()); val = itr == args.begin() ? cval :
(op(val, cval) ? val : cval); } else if (itr->is_list()) { int64_t fval =
apply_arith_basic(op, itr->as_list()); val = itr == args.begin() ? fval :
(op(val, fval) ? val : fval); } else { throw torrent::input_error("Wrong type
supplied to apply_arith_basic.");
        }
    }
...
apply_arith_count.");
    ...
    for (torrent::Object::list_const_iterator itr = args.begin(), last =
args.end(); itr != last; itr++) { switch (itr->type()) { case
torrent::Object::TYPE_VALUE: case torrent::Object::TYPE_STRING: val++; break;
            case torrent::Object::TYPE_LIST:
                val += apply_arith_count(itr->as_list());
                break;
            default:
                throw torrent::input_error("Wrong type supplied to
apply_arith_count.");
        }
    }
...
int64_t apply_arith_other(const char* op, const torrent::Object::list_type&
args) { if (args.size() == 0) throw torrent::input_error("Wrong argument count
in apply_arith_other.");

    if (strcmp(op, "average") == 0) {
        return (int64_t)(apply_math_basic(op, std::plus<int64_t>(), args) /
apply_arith_count(args)); } else if (strcmp(op, "median") == 0) {
        std::vector<int64_t> result = as_vector(args);
        return (int64_t)rak::median(result.begin(), result.end());
    } else {
        throw torrent::input_error("Wrong operation supplied to
apply_arith_other.");
    }
}
*/
...
initialize_command_pyroscope() {
  ...
  // these are merged into 0.9.8+ mainline! (well, maybe, PRs are mostly
  // ignored)
  CMD2_ANY_LIST("system.random", &apply_random);
  // included upstream CMD2_ANY_LIST("d.multicall.filtered",
  ...
  CMD2_ANY_LIST("string.equals",
                std::bind(&cmd_string_compare, 0, std::placeholders::_2));
  CMD2_ANY_LIST("string.startswith",
                std::bind(&cmd_string_compare, 1, std::placeholders::_2));
  CMD2_ANY_LIST("string.endswith",
                std::bind(&cmd_string_compare, 2, std::placeholders::_2));
  CMD2_ANY_LIST("string.strip",
                std::bind(&cmd_string_strip, 0, std::placeholders::_2));
  CMD2_ANY_LIST("string.lstrip",
                std::bind(&cmd_string_strip, -1, std::placeholders::_2));
  CMD2_ANY_LIST("string.rstrip",
                std::bind(&cmd_string_strip, 1, std::placeholders::_2));
  CMD2_ANY_LIST("string.lpad",
                std::bind(&cmd_string_pad, false, std::placeholders::_2));
  CMD2_ANY_LIST("string.rpad",
                std::bind(&cmd_string_pad, true, std::placeholders::_2));
  ...
  /* included upstream
      // math.* group
      CMD2_ANY_LIST("math.add", std::bind(&apply_math_basic, "math.add",
     std::plus<int64_t>(), std::placeholders::_2)); CMD2_ANY_LIST("math.sub",
     std::bind(&apply_math_basic, "math.sub", std::minus<int64_t>(),
     std::placeholders::_2)); CMD2_ANY_LIST("math.mul",
     std::bind(&apply_math_basic, "math.mul", std::multiplies<int64_t>(),
     std::placeholders::_2)); CMD2_ANY_LIST("math.div",
     std::bind(&apply_math_basic, "math.div", std::divides<int64_t>(),
     std::placeholders::_2)); CMD2_ANY_LIST("math.mod",
     std::bind(&apply_math_basic, "math.mod", std::modulus<int64_t>(),
     std::placeholders::_2)); CMD2_ANY_LIST("math.min",
     std::bind(&apply_arith_basic, std::less<int64_t>(),
     std::placeholders::_2)); CMD2_ANY_LIST("math.max",
     std::bind(&apply_arith_basic, std::greater<int64_t>(),
     std::placeholders::_2)); CMD2_ANY_LIST("math.cnt",
     std::bind(&apply_arith_count, std::placeholders::_2));
      CMD2_ANY_LIST("math.avg", std::bind(&apply_arith_other, "average",
     std::placeholders::_2)); CMD2_ANY_LIST("math.med",
  ...
  */
  ...
  // system.has.*
  CMD2_ANY_STRING("system.has",
                  std::bind(&cmd_system_has, std::placeholders::_2));
  CMD2_ANY("system.has.list", std::bind(&cmd_system_has_list));
  CMD2_ANY("system.has.private_methods",
           std::bind(&cmd_system_has_methods, false));
  CMD2_ANY("system.has.public_methods",
           std::bind(&cmd_system_has_methods, true));
  CMD2_ANY("system.client_version.as_value",
           std::bind(&cmd_system_client_version_as_value));

  // d.custom.* extensions
  CMD2_DL_LIST("d.custom.set_if_z",
               std::bind(&cmd_d_custom_set_if_z,
                              std::placeholders::_1,
                              std::placeholders::_2));
  CMD2_DL_LIST("d.custom.erase",
               std::bind(&cmd_d_custom_erase,
                              std::placeholders::_1,
                              std::placeholders::_2));
  CMD2_DL_STRING("d.custom.toggle",
                 std::bind(&cmd_d_custom_toggle,
                                std::placeholders::_1,
                                std::placeholders::_2));
  CMD2_DL_STRING("d.custom.as_value",
                 std::bind(&retrieve_d_custom_as_value,
                                std::placeholders::_1,
                                std::placeholders::_2));
  ...
  CMD2_ANY("throttle.names", std::bind(&cmd_throttle_names));
  CMD2_DL("d.tracker_domain",
          std::bind(&cmd_d_tracker_domain, std::placeholders::_1));
  CMD2_DL(
    "d.tracker_scrape.downloaded",
    std::bind(&cmd_d_tracker_scrape_info, 1, std::placeholders::_1));
  CMD2_DL(
    "d.tracker_scrape.complete",
    std::bind(&cmd_d_tracker_scrape_info, 2, std::placeholders::_1));
  CMD2_DL(
    "d.tracker_scrape.incomplete",
    std::bind(&cmd_d_tracker_scrape_info, 3, std::placeholders::_1));

  CMD2_ANY_STRING(
    "log.messages",
    std::bind(&cmd_log_messages, std::placeholders::_2));
  CMD2_ANY_P("import.return", &cmd_import_return);
  CMD2_ANY("do",
           std::bind(
             &cmd_do, std::placeholders::_1, std::placeholders::_2));
