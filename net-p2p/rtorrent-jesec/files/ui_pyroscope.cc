...
// definition from display/window_download_list.cc that is not in the header
// file
...
int ratio_col[] = {
  ps::COL_PROGRESS0,   ps::COL_PROGRESS20, ps::COL_PROGRESS40,
  ps::COL_PROGRESS60,  ps::COL_PROGRESS80, ps::COL_PROGRESS100,
  ps::COL_PROGRESS120,
};

// ps::COL_PRIO
static int col_idx_prio[] = { ps::COL_PROGRESS0,
                              ps::COL_PROGRESS60,
                              ps::COL_INFO,
                              ps::COL_PROGRESS120 };

// ps::COL_STATE
static int col_idx_state[] = { ps::COL_PROGRESS0,
                               ps::COL_PROGRESS0,
                               ps::COL_PROGRESS80,
                               ps::COL_PROGRESS100 };
...
// list of color configuration variables, the order MUST correspond to the
// ColorKind enum
...
// get custom field containing a long (time_t)
unsigned long
get_custom_long(core::Download* d, const char* name) {
  try {
    return atol(d->bencode()
                  ->get_key("rtorrent")
                  .get_key("custom")
                  .get_key_string(name)
                  .c_str());
...
// get custom field containing a string
std::string
get_custom_string(core::Download* d, const char* name) {
  try {
    return d->bencode()
      ->get_key("rtorrent")
      .get_key("custom")
...
// get a value from arg, either parsing from a string, or arg already being a
// value
int64_t
parse_value_arg(const torrent::Object& arg) {
  if (arg.is_string()) {
    int64_t result;
  ...
  return arg
    .as_value(); // this will throw if other types than string/value are passed
}

// convert absolute timestamp to approximate human readable time diff (5 chars
// wide)
std::string
elapsed_time(unsigned long dt, unsigned long t0) {
  if (dt == 0)
    return std::string("⋆ ⋆⋆ ");

  const char*   unit[]      = { "”", "’", "h", "d", "w", "m", "y" };
  unsigned long threshold[] = { 1,         60,         3600,        86400,
                                7 * 86400, 30 * 86400, 365 * 86400, 0 };

  int dim = 0;
  dt      = (t0 ? t0 : time(NULL)) - dt;
  if (dt == 0)
    return std::string("⋅ ⋅⋅ ");
  while (threshold[dim] && dt >= threshold[dim])
    ++dim;
  if (dim)
    --dim;
  ...
  if (val < 10.0 && dim) {
    snprintf(buffer,
             ...
             int(val),
             unit[dim],
             int(dt % threshold[dim] / threshold[dim - 1]),
             unit[dim - 1]);
  } else {
...
// set bit 8 of format and 0 values will return a whitespace string of the
// correct length.
std::string
human_size(int64_t bytes, unsigned int format = 0) {
  if (format & 8 && bytes <= 0)
    return std::string((format & 7) ? 4 : 6, ' ');
  format &= 7;

  int  exp;
  char unit;

  if (bytes < (int64_t(1000) << 10)) {
    exp  = 10;
    unit = 'K';
  } else if (bytes < (int64_t(1000) << 20)) {
    exp  = 20;
    unit = 'M';
  } else if (bytes < (int64_t(1000) << 30)) {
    exp  = 30;
    unit = 'G';
  } else {
    exp  = 40;
  ...
  if (format > 2)
    format = 0;
  if (format == 2 and value >= 9.949999)
    format = 1;
  if (format == 1)
    value = int(value + 0.50002);
  snprintf(buffer, sizeof(buffer), formats[format], value, unit);

  return std::string(buffer);
}

// split a given string into words separated by delim, and add them to the
// provided vector
void
split(std::vector<std::string>& words, const char* str, char delim = ' ') {
  do {
...
void
            ui_pyroscope_canvas_init(); // forward
static bool color_init_recursion = false;

// create color map from configuration strings
void
ui_pyroscope_colormap_init() {
  // if in early startup stage (configuration), then init the screen so we can
  // query system constants
  if (!get_colors()) {
    if (color_init_recursion) {
      color_init_recursion = false;
      control->core()->push_log(
        "Terminal color initialization failed, does your terminal have none?!");
    } else {
  ...
  for (int k = 1; k < ps::COL_MAX; k++) {
    ...
    for (size_t i = 0; i < words.size(); i++) { // look at all the words
      if (words[i] == "bold")
        attr |= A_BOLD;
      else if (words[i] == "standout")
        attr |= A_STANDOUT;
      else if (words[i] == "underline")
        attr |= A_UNDERLINE;
      else if (words[i] == "reverse")
        attr |= A_REVERSE;
      else if (words[i] == "blink")
        attr |= A_BLINK;
      else if (words[i] == "dim")
        attr |= A_DIM;
      else if (words[i] == "on") {
      ...
      else if (words[i] == "gray")
        col[col_idx] = bright ? 7 : 8; // bright gray is white
      else if (words[i] == "bright")
        bright = 8;
    ...
    if ((col[0] != -1 && col[0] >= get_colors()) ||
        (col[1] != -1 && col[1] >= get_colors())) {
      char buf[33];
      sprintf(buf, "%d", get_colors());
      Canvas::cleanup();
      throw torrent::input_error(col_def + ": your terminal only supports " +
                                 buf + " colors.");
    }
  ...
  // now make copies of the basic colors with the "odd" and "even" definitions
  // mixed in
  for (int k = 1; k < ps::COL_MAX; k++) {
    short fg, bg;
...
// offset into the color index table, depending on whether this is an odd or
// even item
static int
row_offset(core::View* view, Range& range) {
...
torrent::Object
ui_canvas_color_set(const torrent::Object::string_type& arg) {
  ::ui_canvas_color = arg;
  return torrent::Object();
}

int64_t
cmd_d_message_alert(core::Download* d) {
...
std::string
get_active_tracker_alias(torrent::Download* item) {
...
torrent::Object
cmd_d_tracker_alias(core::Download* download) {
  return get_active_tracker_alias(download->download());
}

static void
decorate_download_title(Window*          window,
  ...
  if (int(canvas->width()) <= x_title)
    return;

  // download title color
  int           title_col;
  unsigned long focus_attr =
    range.first == view->focus() ? attr_map[ps::COL_FOCUS] : 0;
  if ((*range.first)->is_done())
    title_col = (active ? D_INFO(item)->up_rate()->rate() ? ps::COL_SEEDING
                                                          : ps::COL_COMPLETE
                        : ps::COL_STOPPED) +
                offset;
  else
    title_col = (active ? D_INFO(item)->down_rate()->rate() ? ps::COL_LEECHING
                                                            : ps::COL_INCOMPLETE
                        : ps::COL_QUEUED) +
                offset;
  canvas->set_attr(
    x_title, pos, -1, attr_map[title_col] | focus_attr, title_col);
  if (hilen && hilite != std::string::npos &&
      x_title + hilite < int(canvas->width())) {
    canvas->set_attr(x_title + hilite,
                     pos,
                     std::min(hilen, int(canvas->width()) - x_title - hilite),
                     (attr_map[title_col] | focus_attr | A_REVERSE) ^ A_BOLD,
                     title_col);
  }

  // show label for active tracker (a/k/a in focus tracker)
  if (int(canvas->width()) <= x_title + NAME_RESERVED_WIDTH + 3)
    return;
  ...
  if (max_len > 0) {
    ...
    // int td_col = active ? ps::COL_INFO : (*range.first)->is_done() ?
    // ps::COL_STOPPED : ps::COL_QUEUED;
    int xpos = canvas->width() - len - 2;
    canvas->print(xpos, pos, "{%s}", url.c_str());
    canvas->set_attr(xpos + 1,
                     pos,
                     len,
                     attr_map[td_col + offset] | focus_attr,
                     td_col + offset);
    canvas->set_attr(xpos,
                     pos,
                     1,
                     (attr_map[td_col + offset] | focus_attr) ^ A_BOLD,
                     td_col + offset);
    canvas->set_attr(canvas->width() - 1,
                     pos,
                     1,
                     (attr_map[td_col + offset] | focus_attr) ^ A_BOLD,
                     td_col + offset);
...
static int
ratio_color(int ratio) {
  int rcol = sizeof(ratio_col) / sizeof(*ratio_col) - 1;
  return ratio_col[std::min(rcol, std::max(0, ratio) * rcol / 1200)];
}

// patch hook for download list canvas redraw of a single item; "pos" is placed
// AFTER the item
void
ui_pyroscope_download_list_redraw_item(Window*          window,
                                       display::Canvas* canvas,
                                       core::View*      view,
                                       int              pos,
                                       Range&           range) {
  ...
  int status_pos = 91;
  int ratio =
    rpc::call_command_value("d.ratio", rpc::make_target(*range.first));
  ...
  if (status_pos < int(canvas->width())) {
    canvas->print(
      ...
      "R:%6.2f [%c%c] %-4.4s  ",
      float(ratio) / 1000.0,
      rpc::call_command_string("d.tied_to_file", rpc::make_target(*range.first))
          .empty()
        ? ' '
        : 'T',
      (rpc::call_command_value("d.ignore_commands",
                               rpc::make_target(*range.first)) == 0)
        ? ' '
        : 'I',
      (*range.first)->priority() == 2
        ? ""
        : rpc::call_command_string("d.priority_str",
                                   rpc::make_target(*range.first))
            .c_str());
    status_pos += 9 + 5 + 5;
  }
  ...
  if (status_pos < int(canvas->width())) {
    ...
    if (!(*range.first)
           ...
           .empty()) {
      // item_status += "T=";
      item_status += rpc::call_command_string("d.throttle_name",
                                              rpc::make_target(*range.first)) +
                     ' ';
    }

    // left-justifying this also overwrites any junk from the original display
    // that we overwrite
  ...
  // [CLOSED]     0,0 /   15,9 MB Rate:   0,0 /   0,0 KB Uploaded:     0,0 MB [
  // 0%] --d --:-- R:nnnnnn [TI] [CLOSED]    0.0K /   0.0K     U/D:   0.0K /
  // 0.0K  Uploaded:   0.0K                     R:  0.00 [T ]
  int         label_pos[] = { 19, 1,  31, 5,  44, 1,   54, 9,   75,
                              1,  79, 1,  91, 2,  100, 1,  103, 1 };
  const char* labels[sizeof(label_pos) / sizeof(int) / 2] = { 0, " U/D:" };
  int         col_active                                  = ps::COL_INFO;
  // int col_active = item->is_open() && item->is_active() ? ps::COL_INFO :
  // (*range.first)->is_done() ? ps::COL_STOPPED : ps::COL_QUEUED;

  // apply basic "info" style, and then revert static text to "label"
  canvas->set_attr(2,
                   pos + 1,
                   canvas->width() - 1,
                   attr_map[col_active + offset],
                   col_active + offset);
  for (size_t label_idx = 0; label_idx < sizeof(label_pos) / sizeof(int);
       label_idx += 2) {
    if (labels[label_idx / 2])
      canvas->print(label_pos[label_idx], pos + 1, labels[label_idx / 2]);
    canvas->set_attr(label_pos[label_idx],
                     pos + 1,
                     label_pos[label_idx + 1],
                     attr_map[ps::COL_LABEL + offset],
                     ps::COL_LABEL + offset);
  }

  // apply progress color to completion indicator
  int pcol = ratio_color(item->file_list()->completed_chunks() * 1000 /
  ...
  // mark active up / down ("focus", plus "seeding" or "leeching"), and dim
  // inactive numbers (i.e. 0)
  canvas->set_attr(
    37,
    pos + 1,
    6,
    attr_map[ps::COL_SEEDING + offset] |
      (D_INFO(item)->up_rate()->rate() ? attr_map[ps::COL_FOCUS] : 0),
    (D_INFO(item)->up_rate()->rate() ? ps::COL_SEEDING : ps::COL_LABEL) +
      offset);
  canvas->set_attr(
    46,
    pos + 1,
    6,
    attr_map[ps::COL_LEECHING + offset] |
      (D_INFO(item)->down_rate()->rate() ? attr_map[ps::COL_FOCUS] : 0),
    (D_INFO(item)->down_rate()->rate() ? ps::COL_LEECHING : ps::COL_LABEL) +
      offset);

  // mark non-trivial messages
  if (!(*range.first)->message().empty() &&
      (*range.first)->message().find("Tried all trackers") ==
        std::string::npos) {
    canvas->set_attr(
      1, pos, 1, attr_map[ps::COL_ALARM + offset], ps::COL_ALARM + offset);
    canvas->set_attr(
      1, pos + 1, 1, attr_map[ps::COL_ALARM + offset], ps::COL_ALARM + offset);
    canvas->set_attr(
      1, pos + 2, -1, attr_map[ps::COL_ALARM + offset], ps::COL_ALARM + offset);
  }
}

torrent::Object
ui_column_spec(rpc::target_type                  target,
  ...
  const torrent::Object::map_type& column_defs =
    control->object_storage()->get_str("ui.column.render").as_map();
...
torrent::Object
ui_column_hide(rpc::target_type                  target,
               const torrent::Object::list_type& args) {
...
torrent::Object
ui_column_show(rpc::target_type                  target,
               const torrent::Object::list_type& args) {
  for (torrent::Object::list_const_iterator itr  = args.begin(),
                                            last = args.end();
       itr != last;
...
torrent::Object
ui_column_is_hidden(rpc::target_type                  target,
                    const torrent::Object::list_type& args) {
  if (args.size() != 1) {
    throw torrent::input_error(
      "ui.column.is_hidden takes exactly one argument!");
  }
...
render_columns(bool                             headers,
  ...
  for (cols_itr = column_defs.begin(); cols_itr != last_col; ++cols_itr) {
    ...
    if (*header_pos++ != ':')
      continue; // 2nd field is missing
    if (column_hidden.count(colidx))
    ...
    if (*header_text == 'C') {
      int x = 0;
      while (header_text[x] && header_text[x] != ':')
        x++;
      color_def.assign(header_text, x);
      header_text += x;
    }
    if (*header_text++ != ':')
      continue; // Header text is missing
    ...
    if (headers) {
      ...
      try {
        ...
        // Rows will rotate through the error string (assuming it is thrown for
        // each row)
        char buf[10];
        int  what_pos = *e.what() ? (pos - CANVAS_POS_1ST_ITEM) * header_len %
                                     strlen(e.what())
                                  : 0;
      ...
      // canvas->print(column, pos, " %s ", ui_canvas_color);  // debug: print
      // color index

      // apply colorization
      if (ui_canvas_color.empty()) {
        canvas->set_attr(column,
                         pos,
                         header_len,
                         attr_map[ps::COL_INFO + offset],
                         ps::COL_INFO + offset);
      } else {
        int attr_col = column;
        for (const char* ptr = ui_canvas_color.c_str();
             *ptr && *ptr++ == 'C';) {
          char* next     = 0;
          int   attr_idx = (int)strtol(ptr, &next, 10);
          if (next == ptr)
            break;
          ptr = next;
          if (*ptr != '/')
          ...
          if (item) {
            ...
            switch (attr_idx) {
              ...
              case ps::COL_PRIO:
                attr_idx =
                  col_idx_prio[std::min(3U, (uint32_t)item->priority())];
                break;
              case ps::COL_STATE:
                attr_idx =
                  col_idx_state[(item->is_open() << 1) | item->is_active()];
                break;
              case ps::COL_RATIO:
                attr_idx =
                  ratio_color(rpc::call_command_value("d.ratio", target));
                break;
              case ps::COL_PROGRESS:
                attr_idx = ratio_color(item->file_list()->completed_chunks() *
                                       1000 / item->file_list()->size_chunks());
                break;
              case ps::COL_ALERT: // COL_ALARM is the actual color, this is the
                                  // dynamic one
                bool has_alert = !item->message().empty() &&
                                 item->message().find("Tried all trackers") ==
                                   std::string::npos;
                bool no_data =
                  item->message().find("no data") != std::string::npos;
                attr_idx = no_data     ? ps::COL_PROGRESS0
                           : has_alert ? ps::COL_ALARM
                                       : ps::COL_INFO;
                break;
            }
          }
          ...
          int attr_len = (int)strtol(ptr + 1, &next, 10);
          if (next == ptr)
            break;
          ptr = next;
          if (attr_idx && attr_len) {
            if (attr_idx >= ps::COL_MAX)
              attr_idx = ps::COL_ALARM;
            canvas->set_attr(attr_col,
                             pos,
                             attr_len,
                             attr_map[attr_idx + offset],
                             attr_idx + offset);
            attr_col += attr_len;
          }
        }
...
bool
ui_pyroscope_download_list_redraw(Window*          window,
                                  display::Canvas* canvas,
                                  core::View*      view) {
  // show "X of Y"
  if (canvas->width() >= X_OF_Y_CANVAS_MIN_WIDTH) {
    ...
    if (item_idx == view->size())
      ...
      canvas->print(
        canvas->width() - 16, 0, "[%5d of %-5d]", item_idx + 1, view->size());
  }
  canvas->set_attr(0, 0, -1, attr_map[ps::COL_TITLE], ps::COL_TITLE);

  if (is_collapsed.find(view->name()) == is_collapsed.end() ||
      !is_collapsed[view->name()])
  ...
  std::transform(
    find_term.begin(), find_term.end(), find_term.begin(), ::tolower);

  // Render header line
  canvas->print(0, pos, "⇳ ");
  int custom_width = render_columns(
    true, narrow, rpc::make_target(), 0, canvas, column, pos, 0, column_defs);
  if (custom_width < 0) { // enter narrow mode
    canvas->print(
      x_base,
      pos,
      "%s",
      std::string(canvas->width() - x_base, ' ').c_str()); // clean slate
    narrow       = true;
    custom_width = render_columns(
      true, narrow, rpc::make_target(), 0, canvas, column, pos, 0, column_defs);
  }
  column += custom_width;
  canvas->print(column, pos, " Name   ");
  column += NAME_RESERVED_WIDTH;
  if (int(canvas->width()) - 8 > column)
    canvas->print(canvas->width() - 8, pos, " Tracker");
  canvas->set_attr(
    0, pos, -1, attr_map[ps::COL_LABEL], ps::COL_LABEL); // header line unicolor
  ...
  if (network_history_depth) {
    ...
    canvas->set_attr(
      0, pos + 1, -1, attr_map[ps::COL_LEECHING], ps::COL_LEECHING);
  }

  // define iterator range
  Range range = torrent::utils::advance_bidirectional(
    view->begin_visible(),
    view->focus() != view->end_visible() ? view->focus()
                                         : view->begin_visible(),
    view->end_visible(),
  ...
  while (range.first != range.second) {
    ...
    canvas->set_attr(1,
                     ...
                     -1,
                     attr_map[col_active + offset],
                     col_active + offset); // base color, whole line
    column = x_base;
    render_columns(false,
                   narrow,
                   rpc::make_target(d),
                   d,
                   canvas,
    ...
    if (int(canvas->width()) > column) {
      ...
      canvas->print(
        ...
        u8_chop(displayname.empty() ? d->info()->name() : displayname.c_str(),
                canvas->width() - column - 1)
          .c_str());
      size_t hilite = std::string::npos;
      if (!find_term.empty()) {
        if (displayname.empty())
          displayname = d->info()->name();
        std::transform(displayname.begin(),
                       displayname.end(),
                       displayname.begin(),
                       ::tolower);
        hilite = displayname.find(find_term);
      }
      decorate_download_title(window,
                              canvas,
                              view,
                              pos,
                              range,
...
void
ui_pyroscope_statusbar_redraw(Window* window, display::Canvas* canvas) {
...
torrent::Object
cmd_view_collapsed_toggle(const torrent::Object::string_type& args) {
  ...
  is_collapsed[view_name] = is_collapsed.find(view_name) == is_collapsed.end()
                              ? true
                              : !is_collapsed[view_name];
...
rpc::object_storage::set_color_string(const torrent::raw_string& key,
...
// Traffic history
int
network_history_depth_get() {
  return network_history_depth;
}

torrent::Object
network_history_depth_set(int arg) {
  if (network_history_depth) {
...
void
network_history_format(std::string& buf, char kind, uint32_t* data) {
  uint32_t samples =
    std::min(network_history_count, (uint32_t)network_history_depth);
  ...
  snprintf(buffer,
           ...
           "%c ⌈%s⌉⌊%s⌋%s",
           kind,
           display::human_size(max_rate, 0).c_str(),
           display::human_size(min_rate, 0).c_str(),
  ...
  if (max_rate > 102) {
    const char* meter[] = { "⠀", "▁", "▂", "▃", "▄", "▅", "▆", "▇", "█" };
    uint32_t base =
      rpc::call_command_value("network.history.auto_scale") ? min_rate : 0;
...
// You MUST call this after changing the auto_scale flag, to see any changes
// immediately!
torrent::Object
network_history_refresh() {
  if (network_history_depth) {
...
torrent::Object
network_history_sample() {
  if (network_history_depth) {
    network_history_up[network_history_count % network_history_depth] =
      torrent::up_rate()->rate();
    network_history_down[network_history_count % network_history_depth] =
      torrent::down_rate()->rate();
...
torrent::Object
cmd_trackers_alias_set_key(rpc::target_type                  target,
                           const torrent::Object::list_type& args) {
  torrent::Object::list_const_iterator itr = args.begin();
  if (args.size() != 2) {
    throw torrent::input_error(
      "trackers.alias.set_key: expecting two arguments!");
  }
...
torrent::Object
cmd_trackers_alias_items(rpc::target_type target) {
  torrent::Object             rawResult = torrent::Object::create_list();
  torrent::Object::list_type& result    = rawResult.as_list();

  for (string_kv_map::const_iterator itr  = tracker_aliases.begin(),
                                     last = tracker_aliases.end();
       itr != last;
       itr++) {
...
torrent::Object
apply_time_delta(const torrent::Object::list_type& args) {
  if (args.size() != 1 && args.size() != 2)
    throw torrent::input_error("convert.time_delta takes 1 or 2 arguments!");
  if (!args.front().is_value())
    throw torrent::input_error(
      "convert.time_delta: time argument must be a value!");
  if (args.size() == 2 && !args.back().is_value())
    throw torrent::input_error(
      "convert.time_delta: time-base argument must be a value!");

  return elapsed_time(args.front().as_value(),
                      args.size() == 2 ? args.back().as_value() : 0L);
}

torrent::Object
apply_human_size(const torrent::Object::list_type& args) {
  if (args.size() != 1 && args.size() != 2)
    throw torrent::input_error("convert.human_size takes 1 or 2 arguments!");

  torrent::Object::value_type bytes = args.front().as_value();
  torrent::Object::value_type format =
    args.size() > 1 ? args.back().as_value() : 2;

  return display::human_size(bytes, format);
}

torrent::Object
apply_magnitude(const torrent::Object::list_type& args) {
  if (args.size() != 1)
...
ui_find_next() {
  ...
  if (dl_view->empty_visible()) {
    ...
    if (!found) {
      control->core()->push_log(
        ("Cannot find anything matching '" + term + "'").c_str());
...
#define PS_VARIABLE_COLOR(key, value)                                          \
  ...
  CMD2_ANY_STRING(key ".set",                                                  \
                  std::bind(&rpc::object_storage::set_color_string,       \
                                 control->object_storage(),                    \
                                 torrent::raw_string::from_c_str(key),         \
                                 std::placeholders::_2));

#define PS_CMD_ANY_FUN(key, func) CMD2_ANY(key, std::bind(&func))

  CMD2_ANY("network.history.depth", std::bind(&network_history_depth_get));
  CMD2_ANY_VALUE_V(
    "network.history.depth.set",
    std::bind(&network_history_depth_set, std::placeholders::_2));
  ...
  CMD2_ANY_STRING(
    ...
    std::bind(&cmd_view_collapsed_toggle, std::placeholders::_2));

  CMD2_ANY_LIST("trackers.alias.set_key", &cmd_trackers_alias_set_key);
  CMD2_ANY(
    "trackers.alias.items",
    std::bind(&cmd_trackers_alias_items, std::placeholders::_1));
  CMD2_DL(
    "d.tracker_alias",
    std::bind(&display::cmd_d_tracker_alias, std::placeholders::_1));

  CMD2_DL(
    "d.message.alert",
    std::bind(&display::cmd_d_message_alert, std::placeholders::_1));

  CMD2_ANY("ui.canvas_color", std::bind(&display::ui_canvas_color_get));
  CMD2_ANY_STRING(
    "ui.canvas_color.set",
    std::bind(&display::ui_canvas_color_set, std::placeholders::_2));
  ...
  CMD2_ANY("ui.column.hidden.list",
           std::bind(&display::ui_column_hidden_list));
  CMD2_ANY("ui.column.sacrificial.list",
           std::bind(&display::ui_column_sacrificial_list));
  CMD2_VAR_VALUE("ui.column.sacrificed", 0);

  CMD2_ANY("ui.find.next", std::bind(&ui_find_next));
  ...
  CMD2_ANY_LIST("convert.time_delta",
                std::bind(&apply_time_delta, std::placeholders::_2));
  CMD2_ANY_LIST("convert.human_size",
                std::bind(&apply_human_size, std::placeholders::_2));
  CMD2_ANY_LIST("convert.magnitude",
                std::bind(&apply_magnitude, std::placeholders::_2));
  ...
  init_commands.append(
    ...
    "branch = (ui.column.sacrificed), ((ui.column.sacrificed.set, 0)), "
    "((ui.column.sacrificed.set, 1)) ; "
    ...
    "schedule2 = column_sacrificed_toggle, 0, 0, "
    "((ui.bind_key,download_list,/,ui.column.sacrificed.toggle=))\n"
    ...
    "schedule2 = ui_find_next_f,  0, 0, ((ui.bind_key, download_list, F,    "
    "\"ui.find.next=\"))\n"
    "schedule2 = ui_find_next_f3, 0, 0, ((ui.bind_key, download_list, 0413, "
    "\"ui.find.next=\"))\n"
    ...
    // TODO: copy (parts of) timestamp cfg here
    // (~/.pyroscope/rtorrent.d/timestamps.rc)
    ...
    "    ((string.map, ((cat, ((d.is_open)), ((d.is_active)))), {00, \"▪ \"}, "
    "{01, \"▪ \"}, {10, \"╍ \"}, {11, \"▹ \"}))\n"
    ...
    // Scrape info (↺ ⤴ ⤵)
    "method.set_key = ui.column.render, \"400:?3C23/3: ↺ \", "
    "((convert.magnitude, ((d.tracker_scrape.downloaded)) ))\n"
    "method.set_key = ui.column.render, \"410:?3C24/3: ⤴ \", "
    "((convert.magnitude, ((d.tracker_scrape.complete)) ))\n"
    "method.set_key = ui.column.render, \"420:?3C14/3: ⤵ \", "
    "((convert.magnitude, ((d.tracker_scrape.incomplete)) ))\n"
    ...
    "method.set_key = ui.column.render, \"510:3C28/3:℞  \", "
    "((convert.magnitude, ((d.peers_connected)) ))\n"

    // Up|Leech Time / Down|Completion or Loaded Time
    // TODO: Could use "d.timestamp.started" and "d.timestamp.finished" here,
    // but need to check
    //       when they were introduced, and if they're always set (e.g. what
    //       about fast-resumed items?)
    ...
    "                              ((value, ((d.custom.if_z, tm_started, "
    "((d.custom, tm_loaded)) )) )) ))"
    "    ))\n"
    "method.set_key = ui.column.render, \"530:6C90/6:∇⋮ ◷  \","
    "    ((if, ((d.down.rate)),"
    "        ((convert.human_size, ((d.down.rate)), ((value, 10)) )),"
    "        ((convert.time_delta, ((value, ((d.custom.if_z, tm_completed, "
    "((d.custom, tm_loaded)) )) )) ))"
    "    ))\n"
    ...
    "                     ((math.div, ((math.mul, ((d.completed_chunks)), "
    "10)), ((d.size_chunks)) )) )), 2, \"✔ \"))\n"
    // "  ⠁ ⠉ ⠋ ⠛ ⠟ ⠿ ⡿ ⣿ ❚ "
    // ⠀"  ▁ ▂ ▃ ▄ ▅ ▆ ▇ █ "
    "method.set_key = ui.column.render, \"920:3C93/3:☯  \","
    "    ((string.substr, \"☹ ➀ ➁ ➂ ➃ ➄ ➅ ➆ ➇ ➈ ➉ \", ((math.mul, 2, "
    "((math.div, ((d.ratio)), 1000)) )), 2, \"⊛ \"))\n"
    // "☹ ➀ ➁ ➂ ➃ ➄ ➅ ➆ ➇ ➈ ➉ " "😇 "
    // "☹ ① ② ③ ④ ⑤ ⑥ ⑦ ⑧ ⑨ ⑩ "
    // "☹ ➊ ➋ ➌ ➍ ➎ ➏ ➐ ➑ ➒ ➓ "
