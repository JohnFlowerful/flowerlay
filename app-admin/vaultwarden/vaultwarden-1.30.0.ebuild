...
# using this unreleased version of rocket will generate 'experimental' warnings
ROCKET_COMMIT="ce441b5f46fdf5cd99cb32b8b8638835e4c2a5fa"
declare -A GIT_CRATES=(
	[rocket]="https://github.com/SergioBenitez/Rocket;${ROCKET_COMMIT};Rocket-%commit%/core/lib"
	[rocket_ws]="https://github.com/SergioBenitez/Rocket;${ROCKET_COMMIT};Rocket-%commit%/contrib/ws"
)
...
DEPEND="
	dev-libs/openssl:0=
	>=app-admin/vaultwarden-web-vault-2023.10.0
	mysql? ( virtual/mysql )
...
src_prepare() {
	...
	# override rocket path
	# the cargo eclass doesn't handle the current vaultwarden cargo.toml
	sed -r "s|^rocket = \{ git.*|rocket = { path = \"${WORKDIR}/Rocket-${ROCKET_COMMIT}/core/lib\" }|" \
		-i "Cargo.toml" || die
...
pkg_postinst() {
	if [[ $(grep 'WEBSOCKET_ENABLED' "/etc/${PN}.env") ]]; then
		elog "Note: The WebSockets service for live sync has been integrated in the"
		elog "main HTTP server, which means simpler proxy setups that don't require"
		elog "a separate rule to redirect WS traffic to port 3012. Please check the"
		elog "updated examples in the wiki (https://github.com/dani-garcia/vaultwarden/wiki/Proxy-examples)."
		elog "It's recommended to migrate to this new setup as using the old server"
		elog "on port 3012 is deprecated, won't receive new features and will be"
		elog "removed in a future release."
	fi
}
